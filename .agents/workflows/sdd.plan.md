...
4. **Gate 2（計画承認・ペルソナ査定）の完全展開とユーザー承認要請**:
   - 以下の4大要素をユーザーメッセージ内に**完全展開（Full Disclosure）**して停止（Gate 2 HALT）すること：
     ...
     2. **📋 実装計画チェックリスト ＆ 機械的突合エビデンス**: 章節タイトルGrep突合、ナショジオ新要素（早わかり、サイズ比較、**QGIS 2画面タクティカルマップ生成タスク定義＆画像実在・解像度/容量（>100KB）突合**、3大写真）、専門部署別チェックマトリックス（GURPS公式呪文行番号突合、**地理部/視覚部 QGISコマンド・マップ突合**）。
