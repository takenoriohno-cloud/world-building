...
INDEX_URL = "https://seesaawiki.jp/mokugyo/d/%b0%dc%c6%b0%b7%cf%bc%f6%ca%b8"
...
def fetch_page(url):
    ...
    with urllib.request.urlopen(req, timeout=20) as response:
        ...
        # Seesaawiki is EUC-JP
        return content_bytes.decode('euc-jp', errors='replace')

def parse_spell_detail(html):
    # Extract main content div from seesaawiki
    # Usually between <div id="content"...> or <div class="wiki-content"...> and footer
    ...
    # Remove script, style, comments, navigation
    text = clean_html(content_html)
    
    # Try to extract Description, Item / Enchantment
    return text
...
def main():
    ...
    spells = []
    # Tables 1 to 5 contain spells
    for t_idx in range(1, min(6, len(parser.tables))):
        table = parser.tables[t_idx]
        if not table or len(table) < 2:
            continue
        
        headers_row = [c[0] for c in table[0]]
        print(f"Table {t_idx} headers: {headers_row}")
        ...
        for row in table[1:]:
            ...
            # Extract link to spell detail
            # Usually column 1 (呪文名) has the link
            detail_url = ""
            for cell_text, links in row:
                for l_text, l_url in links:
                    if "/mokugyo/d/" in l_url:
                        # Prioritize spell name column
    ...
    print(f"Collected {len(spells)} spells from index.")
    
    # Save partial list
    output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "../../scratch")
    os.makedirs(output_dir, exist_ok=True)
    json_path = os.path.join(output_dir, "movement_spells_data.json")
    
    # Fetch details for each spell
    for idx, s in enumerate(spells):
        print(f"[{idx+1}/{len(spells)}] Fetching detail: {s['japanese_name']} ({s['english_name']})...")
        if s['detail_url']:
            try:
        ...
        # Incremental save every 10 spells
        if (idx + 1) % 10 == 0 or idx == len(spells) - 1:
            with open(json_path, 'w', encoding='utf-8') as f:
                json.dump(spells, f, ensure_ascii=False, indent=2)
            print(f"  Saved progress ({idx+1} spells)")

    print(f"\nAll spells fetched successfully! Saved to {json_path}")
