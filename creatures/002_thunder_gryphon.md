...
| **資源・管理区分** | **要処理種（Class-B）**[^zext_1]<br>※管理ステータス: **防空制限空域指定種 / 認可魔獣解体技師による生体放電器官「ライデン腺」の絶縁摘出・完全アース放電処理必須（下処理完了後の肉・羽毛のみ合法高額流通認可）** |
...
- **ヘロドトス『歴史（*Historiai*）』第4巻第13・27節（古代ギリシャ）**:
  紀元前5世紀の歴史家ヘロドトスは、中央アジア・スキタイ北方の未踏山脈に生息する怪鳥「グリプス（*γρύψ*）」について記録している。「黄金を守護する鋭い嘴を持った四足の怪鳥であり、黄金を採掘しようとする隻眼の遊牧民族アリマスポイ族と絶え間ない闘争を繰り広げている」と記述され、金鉱脈と猛禽・巨獣キメラの結びつきを示す最古の学術記録である[^schulz_1]。
...
- **骨格と筋肉構造（DR値）**:
  ...
  - 胴体・後肢は強靭な獣皮（**DR 4**）、翼および前肢は導電性硬質羽毛・ケラチン装甲（**DR 6**）。
- **鉤爪と嘴の力学**:
...
- **感覚器官・生体放電器官**:
  - **生体放電器官「ライデン腺（*Organum Fulgurans*）」**: 竜骨突起直下の胸腔中央に位置する高電圧生体コンデンサ器官。心筋収縮の生体電位と、導電性風切羽が大気中のイオンを摩擦収集することで蓄電。最大 **100万ボルト（約1.2メガジュール）** のパルス放電を前肢鉤爪および嘴からアーク放電として射出可能[^kuroda_1]。
  - **超望遠電磁複眼（*Oculus Electromagneticus*）**: 網膜にマナ受容細胞と電磁波感知ピクセルが蜂の巣状に配列。5km上空から地上の小動物の体温、高圧電線の微弱漏洩電磁波、航空機のジェットエンジン排気熱を鮮明に識別。

### 3.2 ライフステージと繁殖・育児ドラマ
1. **幼体・若齢期（Juvenile / 0〜3年）**:
   - 2〜3年に一度、1〜2個の琥珀色の大卵（長径25cm・重さ4.5kg）を産卵。雌雄交代で45日間抱卵する。
   - 孵化した幼体は最初の1年間、前年生まれの若鳥（ヘルパー）から餌と飛び方を教わる社会性育児を行う。
2. **成熟個体（Adult / 4〜40年）**:
   - 一夫一妻の強固なペアを形成し、標高3,000m以上の断崖絶壁に直径4〜6mの巨大な巣を構築。縄張り半径は 40〜60km に及ぶ。
   - 水平巡航速度は時速 110〜140km、雷雲からの急降下ダイブ（Dive Attack）時には時速 **240km（約66.7 m/s / Move 24）** に達する。
3. **主級・変異個体（Elder Alpha / 50年以上）**:
   - 翼開長13.5m超（SM +3 / 体重850kg）。《連鎖電撃》や広域雷雲励起を操り、天災級航空脅威（Threat Level: IV）に達する。
...
### 4.1 食性・捕食行動
- **主食・栄養源**:
  高山性変異ウシ、ヘラジカ、ビッグホーン、山麓の剛毛巌猪（オクタマイワシシ）。また高圧送電線や航空機の排気熱・電磁波から生体給電を行う。
- **急降下強襲戦術**:
  雷雲の中に身を潜め、電磁複眼で獲物をロックオン。時速240kmで急降下し、100万ボルトのアーク放電を伴う両前肢鉤爪で獲物の脊椎を粉砕する。

```mermaid
flowchart LR
    A["【高山生態系 / 雷雲電荷】"] --> B["【シロエリワシジシ】"]
    B -->|幼体を捕食| C["【第001号 オクタマイワシシ】"]
    B -->|縄張り威嚇棲み分け| D["【第010号 アラビアルフ】"]
    B -->|電磁強襲・航空交戦| E["【メガコーポ航空コンボイ】"]

    style A fill:#1e293b,stroke:#facc15,stroke-width:2px,color:#ffffff;
    style B fill:#0f172a,stroke:#38bdf8,stroke-width:2px,color:#ffffff;
    style C fill:#451a03,stroke:#f97316,stroke-width:2px,color:#ffffff;
    style D fill:#334155,stroke:#e11d48,stroke-width:2px,color:#ffffff;
    style E fill:#064e3b,stroke:#34d399,stroke-width:2px,color:#ffffff;
```

### 4.2 魔獣間クロス・リレーション（相互作用）
- **第001号 オクタマイワシシ（剛毛巌猪）**:
  山麓に生息するオクタマイワシシの幼体〜若齢個体は主要な高山タンパク源。成体雄はシリカ剛毛で防御するため、急降下からの頭蓋圧砕を狙う。
- **第010号 アラビアルフ（タイタノロク）**:
...
- **防空機関砲・対空ミサイル迎撃**:
  本種は風切羽による電磁シールド（DR 6）と強靭な骨格（DR 4）を持つため、小口径の5.56mm/7.62mm小銃弾は弾き返される。自衛隊の87式自走高射機関砲（35mm）、米軍・アレス社の **20mm M61 バルカンCIWS**、および **FIM-92 スティンガー対空ミサイル** の近接信管爆風による迎撃プロトコルが標準化されている[^jinnai_1]。
- **メガコーポ利権**:
  アレス・マクロテクノロジー社が、導電性風切羽から抽出される生体グラフェン繊維を用いた「電磁パルス（EMP）防護航空装甲」および「対魔術防弾ベスト」の独占特許を保有。
...
- **生体励起公式呪文（公式アーカイブ突合エビデンス）**:
  ...
  - **《連鎖電撃》（Chain Lightning / `world/magic/spells/air.md:69`）**:
    アルファ個体が放つ高圧アーク放電。初弾命中後、周囲5m以内の電子機器やドローンへ電弧が連鎖跳躍し、回路を瞬時に焼き切る。
...
- **生体媒介原則（Bio-Medium Principle）**:
  高電圧放電および電磁シールドは、自身の導電性ケラチン羽毛とライデン腺を生体媒介として循環。周囲の物理電子回路には物理的な雷撃破壊としてのみ作用する。
...
- **超低周波羽ばたき音（14 Hz / 95 dB）**:
  巨翼（翼開長11m）が空気を切り裂く際に生じるインフラサウンド。人間の内耳三半規管および胸腔を共振させ、強烈な圧迫感と平衡感覚喪失を引き起こす[^hasumi_1]。
- **落雷放電衝撃波（130 dB SPL）**:
...
- **アンチ・アビス雷霊純性**:
  本種が操る高電圧生体プラズマ放電（純度99.8%の風霊・雷霊マナ）は、アビス特異点から湧出する低周波精神汚染波を物理的・霊的に焼き払い、変異や精神汚染を一切受け付けない強い浄化耐性を持つ。
...
#### 【通常成体（Standard Adult）】
```yaml
Name: シロエリワシジシ［白襟鷲獅子］（Gryphus fulgurans / Thunder Gryphon）
Archetype: 古代雷霊キメラ巨獣（猛禽＋ネコ科肉食獣）
Point Total: 380 pt
Threat Level: III (高脅威度 / 防空機関砲・対空ミサイル対応)
...
Basic Parameters:
  Basic Speed: 6.75 [0]
  Basic Move: 6 [0] / 飛翔巡航 Move 14 / 急降下ダイブ Move 24（時速240km）
  Size Modifier (SM): +2 (体長 3.5m, 翼開長 10.5m, 体重 520kg)
  Dodge: 11 (空中旋回時) / 受け 10 (前肢鉤爪)

Damage:
  Melee Attack: 打撃 2d+2 / 振 5d
  Claw Strike (炭素強化鉤爪): 2d+2 切り・刺し [Striker / DR 6]
  Bite Strike (巨大鉤嘴): 2d+2 刺し [咬合力 1,800 PSI]

DR (防護点) & 防御特性:
  - 胴体・後肢: DR 4 (強靭獣皮)
  - 翼・前肢: DR 6 (導電性硬質羽毛・ケラチン装甲)
  - 特殊防御: 風切羽電磁シールド (DR +2 vs 金属投射体) / 完全電気耐性

Traits & Advantages:
  - 生体電撃放射 (Innate Attack 3d 電撃 / 射程 20m / 金属鎧DR無視) [30]
  - 超望遠電磁複眼 (Telescopic Vision 3 / Night Vision 5 / 電磁波感知) [25]
  - アンチ・アビス精神浄化 (Magic Resistance 5 vs 精神汚染) [10]

Disadvantages & Quirks:
  - 要処理種 (Class-B: 死後1時間以内に完全放電処置必須) [-5]
  - 縄張り意識 (Greedy Territory / 航空機への迎撃衝動) [-10]
  - 食欲旺盛 (Increased Consumption 2 / 1日あたり肉25kg) [-20]
  - 野生動物 (Wild Animal) [-30]

Innate Spells (生体励起魔術 - マナ消費はFP):
  - 《電撃 / Lightning》: Skill 15 (FP 2〜3消費 / 2d〜3d電撃)
  - 《飛行 / Flight》: Skill 14 (自重半減・空中機動)
```

#### 【主級・長老個体（Elder Alpha）】
- **身体規模**: SM +3（体長 4.8m / 翼開長 13.5m / 体重 850kg）。
- **能力値修正**: ST 32、HP 35、胴体 DR 6、翼 DR 8。
- **特殊異能**: 《連鎖電撃》（Chain Lightning / 最大4目標電弧跳躍）、雷雲召喚（周囲3km乱気流励起）。

### 6.2 推奨火器・防護装備
- **有効火器・弾薬**: **20mm M61 バルカンCIWS**、**35mm高射機関砲**、**FIM-92 スティンガー対空ミサイル**（近接信管爆風破砕）。小銃弾（5.56mm/7.62mm）は完全無効。
- **必須防護具・センサー**: EMP電磁パルスシールド回路、絶縁耐電圧パイロットスーツ、光電サーマル複合レーダー。

### 6.3 市民防災メモ ＆ 専門家の知恵袋

> [!IMPORTANT]
> **【民間航空・山岳通行者向け防災メモ：サンダーグリフォン遭遇時の退避手順】**
> 1. 航空機運航中に青白いアーク放電スパイクを感知した場合は直ちにアフターバーナーを使用し、防空識別圏（ADIZ）のCIWS援護圏内へ退避してください。
> 2. 高山登山中に超低周波羽ばたき音（内耳の圧迫感）を感知した場合は、直ちに岩陰の窪みに身を伏せ、金属製ストックやピッケルを体から離して接地してください。  
> —— *国際民間航空機関（ICAO）魔導航空安全部 / 防衛省統合幕僚監部*

> [!TIP]
> **【現場専門家・ベテランハンターの知恵袋】**
> *「奴らは雷が大好物だから、スティンガーの熱源追尾が最高のデザートになるのさ。小銃で撃つのは自殺行為だ。20mmバルカンの重厚な掃射で電磁シールドごと叩き落とせ！」*  
> —— **アレス・セキュリティ第4航空警備小隊長（Falcon-Lead）**
...
1. **Step 1: アース放電処置（Earthing Discharge）**: 撃墜後15分以内に、純銅アース接地杭を竜骨突起直下のライデン腺へ打ち込み、大地へ残留電荷（最大100万V）を安全に逃がす[^takatsukasa_1]。
2. **Step 2: 放電完了確認**: 生体電位テスターで 0V を確認。怠ると解体作業者が感電死（3d電撃）する。
3. **Step 3: ライデン腺の絶縁摘出**: シリコン絶縁メスでライデン腺（約3.5kg）を切除し、電磁シールドコンテナに封入。
4. **Step 4: 肉・羽毛の出荷**: Class-B正規証明タグを発行し出荷。
- **市場取引相場**: 絶縁処理済みの新鮮肉はキロ単価 15万〜20万円。1頭分の総取引額は 2,000万〜3,500万円に達する。

### 7.2 伝統料理・メガコーポスペシャリテ（本格レシピ）

```
【料理名】鷲獅子胸肉の瞬間燻製ロースト
- 分類: メガコーポ迎賓館最高級スペシャリテ / 航空要塞伝統ジビエ
- 主材料: シロエリワシジシ胸肉ブロック 500g、サバンナハーブ（タイム、ローズマリー）、桜燻製チップ、ゲランドの岩塩、粗挽き黒胡椒、特製バルサミコ赤ワインソース
- 調理手順:
  1. 【肉の下準備】: 高圧通電処理された胸肉の筋膜を丁寧に引き、岩塩と黒胡椒、ハーブを擦り込んで30分常温に置く。
  2. 【高温瞬間燻製】: 密閉スモーカーに桜チップを敷き、250℃の超高温煙で表面を5分間一気に燻し焼きにする。
  3. 【オーブンロースト】: 160℃のオーブンに移し、中心温度が56℃（ロゼ色）になるまで約20分間じっくり熱を入れる。
  4. 【カッティング・盛り付け】: 10分間休ませて肉汁を落ち着かせた後、厚さ1cmにスライスし、バルサミコ赤ワインソースを添える。
- テイスティングノート:
  高電圧放電刺激によってアミノ酸が凝縮された胸肉は、噛みしめるごとに濃厚なジビエの野性味と微かなピリッとした電気的清涼感が広がる。
```
...
| :--- | :--- | :--- |
| **導電性風切羽（主翼羽毛）** | アレス社製 EMP防護航空装甲、対魔術防弾ベスト「エアロシールド」 | 1羽分（1枚）: 25万〜40万円 |
| **生体コンデンサ「ライデン腺」** | 先端魔導航空機用 高電圧パルス電源、指向性EMP兵器コア | 1器官（完全状態）: 800万〜1,500万円 |
| **炭素強化ケラチン鉤爪** | 高硬度切削工具、超音速ドローン翼端パーツ、儀礼用祭具 | 1爪: 30万〜50万円 |
...
```
【北米UCAS カスケード航空防衛第2セクター交戦記録】
日時: 2026年5月18日 16:42 PST
場所: 北米UCAS カスケード航空防衛第2セクター（高度 3,400m）
記録: アレス・セキュリティ第4航空警備小隊（コールサイン: Falcon-Lead）
--------------------------------------------------------------------------------
...
[^zext_1]: ⚖️ **首席監査官ゼクスト（憲章監査室）**:
    「『自家発電できる巨大ペットとして飼育したい』という申請が年に数件届きますが、すべて門前払いで即時却下しています。結界都市内で11mの翼を広げられた日には、家庭用ブレーカーどころか区の変電所ごと吹き飛んで大停電ですからね。」

[^schulz_1]: 🏛️ **V・シュルツ 特命調査員（法務・古典文献考証部）**:
    「紀元前5世紀のアリマスポイ族から現代のアレス・マクロテクノロジー社まで、人類がグリフォンから黄金と羽毛を奪い合っている構図は2500年間まるで進歩していません。唯一の進歩といえば、アレス社の企業法務部がヘロドトスより遥かに悪辣で分厚い排他的ライセンス契約書を作成できる点くらいでしょう。」

[^kuroda_1]: ⚡ **クリスティナ・黒田 所長（魔導科学・理論研究部）**:
    「100万ボルトのパルス放電器官と聞いて『研究所の裏庭で飼えば電気代タダじゃん！』と目を輝かせたインターン生がいましたが、毎日25kgの高級生肉代と電磁シールドケージの修繕費で会社が秒速で破産します。魔導熱力学と収支計算の講義をもう一度履修してきなさい。」

[^jinnai_1]: 🌍 **陣内 隆文 調査官（地理・環境調査部）**:
    「小銃の5.56mm弾で奴さんを撃ち落とそうとするのは、BB弾でジェット戦闘機に喧嘩を売るようなもんだ。カスケードの現場PMC連中は『奴らは雷が大好物だから、スティンガーの熱源追尾が最高のデザートになるのさ』なんて軽口を叩いてるが、時速240kmで真上からダイブされた時のチビりそうな風圧はとても笑い事じゃないぜ。」

[^takatsukasa_1]: 🐾 **鷹司 冴子 博士（生態・生物調査部）**:
    「アース放電をケチった新米解体師が、触れた瞬間に見事なアフロヘアーになって救急搬送される事例が後を絶ちません。ただ、肉自体は天然の超高圧通電ショックで筋繊維が極限まで熟成されていて本当に絶品なんです。研究室で胸肉の瞬間燻製を試作した日は、普段寄り付かない他学部の教授たちまで匂いに釣られて集まってきましたよ。」

[^hasumi_1]: 📸 **蓮見 蓮 プロデューサー（視覚・音響・資料記録部）**:
    「レーニア山の稜線で超望遠レンズを構えていた時、この14Hzのインフラサウンドを浴びて内臓が洗濯機のようにシェイクされ、昼飯のタコスをリバースしかけました。……でも見てくださいよ、野生写真のあの純白の襟羽と紫電のコントラスト！ 胃薬3錠と引き換えにする価値は十二分にありましたね！」
