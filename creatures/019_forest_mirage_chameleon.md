...
- **上位存在（動物王）の真名・固有名詞**:
  - **真名・固有名詞**: **『フシュー・カァ・ア（Hsshuu-Kaa-Ah）』**
    - **動物のアーキタイプの王（Animal Lord / Beast Lord）**:
      有鱗目カメレオン科の生物自身が威嚇・求愛・自己主張の際に発する3大発声（Vocalizations）の音素構造から成る始原の真名：
      1. 肺胞からの連続呼気摩擦音（咽頭ヒス音）: `Hsshuu-`（フシュー）
      2. 喉袋と下顎骨の瞬間圧搾による共振破裂音: `Kaa-`（カァ）
      3. 舌骨格納時の呼気余韻・極低周波呼吸音: `Ah`（ア）
      人間には単なるシューという威嚇の噴気音や喉鳴らしにしか聞こえないが、カメレオン科の全個体が遺伝子レベル・マナ受容器で認識可能な鳴き声そのものである。上位次元契約（プレーン・パクト）を結ぶ術士は、自身の喉頭と舌骨を生体媒介共鳴させ、この3音素を寸分違わず発音しなければ呼び出すことは叶わない。
  - **アストラル界での称号・階位**: 〈擬態と遅延の爬虫王〉（Lord of Ambush and Temporal Slowness / 獣王界・爬虫類低位領主）
  - **アストラル原型・顕現形態**: アストラル界に座す多面結晶の巨樹に潜む、全長15m級の半透明な多面屈折巨獣。物質界に部分降臨した際は周囲数平方キロメートルの森全体を鏡面化し、数百体の光学的幻影を同時に投射・統率する。
- **プレーン・パクト（次元契約）条件・誓約代償**:
  - **召喚・現出プロトコル**: 生きた樹上性昆虫300匹から抽出した純粋マナ結晶の捧持、および術士が一切の筋肉運動と呼吸を60分間停止する「擬死静止（Feign Death）」状態への同調儀式。
  - **契約代償・誓約制約**: 術士の主観時間の恒久的一部譲渡。契約期間中、術士の代謝および歩行移動速度は爬虫類のように半分以下へ緩慢化するが、代償として動体視力と反射神経が超音速（DX+3相当）へ研ぎ澄まされ、完全光学迷彩の恩恵を受ける。
- **アビス特異点との因果関係**:
  - マダガスカル島東方沖、インド洋アビス海溝から漏出する微小空間歪曲パルスが、アンダシベ熱帯原生林の植物・光霊マナと結合。本種のS-イリドフォア層の超次元光屈折特性を励起・固定化させた直接の誘因となっている。
- **メガコーポの極秘研究・特許利権**:
  - **三ツ菱マナテック**: 表皮グアニンナノ結晶の動的格子制御を軍事転用した「ミラージュ・スーツ」の基本特許（JP-2024-88492-A）を単独保持。東京湾岸夢の島バイオドームにて選抜育種を進行中。
  - **ヤマト重工**: 舌骨加速装置の生体腱ばね力学に着目し、歩兵携行型「超軽量電磁アシスト投射カタパルト」への応用研究を極秘裏に推進。両メガコーポによる生体サンプル・密輸個体の争奪戦が水面下で激化している。
